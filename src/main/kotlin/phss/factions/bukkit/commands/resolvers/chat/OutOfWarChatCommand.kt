...
class OutOfWarChatCommand(
    ...
    override fun invoke(sender: CommandSender, args: Array<out String>) {
        ...
        for (relation in faction.relations.filter { it.type == RelationType.OUT_OF_WAR }) {
        ...
        val format = messages.getMessage("chatFormat.outofwarchat").replaceFactionInfo(faction, plugin).replace("{player}", sender.name).replace("{message}", message)
