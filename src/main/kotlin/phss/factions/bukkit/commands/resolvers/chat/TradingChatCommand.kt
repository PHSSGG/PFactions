...
class TradingChatCommand(
    ...
    override fun invoke(sender: CommandSender, args: Array<out String>) {
        ...
        for (relation in faction.relations.filter { it.type == RelationType.TRADING }) {
        ...
        val format = messages.getMessage("chatFormat.tradingchat").replaceFactionInfo(faction, plugin).replace("{player}", sender.name).replace("{message}", message)
