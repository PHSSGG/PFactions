...
class SetLeaderCommand(
    ...
    override fun invoke(sender: CommandSender, args: Array<out String>) {
        ...
        targetFaction.leader = targetPlayer.uuid

        factionManager.saveFaction(targetFaction)
        userManager.saveUser(targetPlayer)

        sender.sendMessage(messages.getMessage("admin.leaderChanged").replaceFactionInfo(targetFaction, plugin).replace("{player}", targetPlayer.name))
    }
