...
import phss.factions.bukkit.commands.resolvers.UnbanCommand

class FactionUnbanConversation : StringPrompt() {
    ...
    override fun getPromptText(context: ConversationContext): String {
        return (context.plugin as PFactions?)?.messages?.getMessage("conversation.unban") ?: ""
    }
    ...
    override fun acceptInput(context: ConversationContext, name: String?): Prompt? {
        ...
        Bukkit.getScheduler().callSyncMethod(plugin) {
            UnbanCommand(plugin).execute(player, "unban", arrayOf("unban", playerName))
        }
